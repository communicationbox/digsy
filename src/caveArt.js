...
export function caveWall(g, tx, ty, sx, sy, info, time) {
  ...
  /* PARETE che guarda il pavimento: strati orizzontali di roccia, più scura in basso */
  const FH = 20, y0 = sy + TS - FH;
  g.rect(sx, y0 - 1, TS, 1, LN);
  for (let r = 0; r < FH; r += 5) {
    const c = [ROCK.faceL, ROCK.face, ROCK.face, ROCK.faceD][Math.min(3, r / 5 | 0)];
    g.rect(sx, y0 + r, TS, 5, c);
    for (let x = (ty * 3 + r) % 7; x < TS; x += 6 + ((x + r) % 5)) g.rect(sx + x, y0 + r + 4, 3, 1, ROCK.faceD);
  }
  g.rect(sx, y0, TS, 1, ROCK.lite);
  /* ossa fossili incastrate nella roccia: la grotta è piena di storia */
  if (vhash(tx, ty, 470) < 0.12) {
    const x = sx + 6 + Math.floor(vhash(tx, ty, 471) * 14), y = y0 + 6;
    g.rect(x - 1, y - 1, 12, 5, LN); g.rect(x, y, 10, 3, '#d8cfb8'); g.rect(x - 2, y - 2, 4, 4, '#d8cfb8'); g.rect(x + 8, y - 2, 4, 4, '#d8cfb8'); g.px(x, y, '#f4eee0');
  }
  /* vene di cristallo SOLO vicino a un giacimento */
  if (info.nodeNear(tx, ty, 3)) { const x = sx + 4 + Math.floor(vhash(tx, ty, 480) * 20); g.rect(x, y0 + 3, 2, 12, '#3f8a96'); g.rect(x, y0 + 3, 1, 12, '#a6ecf2'); g.rect(x + 2, y0 + 8, 3, 2, '#6fd6e0'); }
  /* stalattiti che pendono dal bordo, con la goccia che cade (fase dal tempo e dalla casella) */
  if (vhash(tx, ty, 490) < 0.45) {
    const x = sx + 6 + Math.floor(vhash(tx, ty, 491) * 18), len = 5 + Math.floor(vhash(tx, ty, 492) * 6);
    for (let k = 0; k < len; k++) { const w = Math.max(1, Math.round(3 * (1 - k / len))); g.rect(x - w, sy + TS + k - 1, w * 2, 1, k === 0 ? ROCK.faceD : ROCK.face); }
    const fall = (time / 1400 + vhash(tx, ty, 493)) % 1;
    if (fall < 0.7) g.rect(x - 1, sy + TS + len + Math.round(fall * 22), 1, 2, '#9fc4d8');
  }
}
...
export function caveFloor(g, tx, ty, sx, sy, info, time) {
  ...
  if (wallAbove && vhash(tx, ty - 1, 490) < 0.45) {
    ...
    if (wet) {
      ...
      for (let k = 0; k < hgt; k++) { const w = Math.max(1, Math.round(5 * (1 - k / (hgt + 2)))); g.rect(x - w - 1, b - k, w * 2 + 2, 1, LN); g.rect(x - w, b - k, w * 2, 1, ROCK.face); g.rect(x - w, b - k, Math.max(1, w >> 1), 1, ROCK.faceL); }
    }
  }
