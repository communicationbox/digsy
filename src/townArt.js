...
/* CONTORNO A COLORE: la lineart non deve essere nera, è la tinta del pezzo molto scurita.
   Con la riga nera attorno a ogni finestra, insegna, comignolo e vetrina la città sembrava
   fatta di adesivi ritagliati (segnalato due volte). */
const ln = (g, c) => sh(g, c, 0.34);
...
export function windowBox(g, x, y, w, h, glass, shutter, night) {
  if (shutter) for (const sx of [x - 7, x + w + 1]) { g.rect(sx, y - 1, 6, h + 2, ln(g, shutter)); g.rect(sx + 1, y, 4, h, shutter); for (let k = 3; k < h; k += 4) g.rect(sx + 1, y + k, 4, 1, sh(g, shutter, 0.7)); }
  g.rect(x - 1, y - 1, w + 2, h + 2, ln(g, '#f3ecda'));
...
export function flowerBox(g, x, y, w, flowers, an = NOAN) {
  g.rect(x - 1, y, w + 2, 6, ln(g, '#8a5f38')); g.rect(x, y + 1, w, 4, '#8a5f38'); g.rect(x, y + 1, w, 1, '#b07c4a');
...
export function wallLamp(g, x, y, night, an = NOAN) {
  g.rect(x, y, 6, 2, ln(g, '#ffe08a')); g.rect(x + 1, y + 2, 4, 7, ln(g, '#ffe08a')); g.rect(x + 2, y + 3, 2, 5, night ? '#ffe08a' : '#e8c34a');
  if (night) {
...
export function hangingSign(g, x, y) {
  const staffa = ln(g, '#8f887a');                                   // il ferro della staffa, non il nero
  g.rect(x - 2, y, 20, 2, staffa); g.rect(x + 2, y + 2, 1, 4, staffa); g.rect(x + 13, y + 2, 1, 4, staffa);
  g.rect(x - 1, y + 6, 18, 14, ln(g, '#d9b98a')); g.rect(x, y + 7, 16, 12, '#d9b98a'); g.rect(x, y + 7, 16, 2, '#efe0bd');
}
...
function rockingChair(g, x, y, an) {
  ...
  for (const [rx, ry] of cells.values()) for (const [ox, oy] of [[1, 0], [-1, 0], [0, 1], [0, -1]])
    if (!cells.has((rx + ox) + ',' + (ry + oy))) g.px(x + rx + ox, y + ry + oy, ln(g, cells.get(rx + ',' + ry)[2]));
...
export function drawStoreFront(g, w, h, BB, glass, night, an = NOAN) {
  ...
  g.rect(w - 30, h - 20, 16, 12, ln(g, '#b98d59')); g.rect(w - 29, h - 19, 14, 10, '#b98d59'); g.rect(w - 29, h - 19, 14, 2, '#d9b98a');
  for (let i = 0; i < 3; i++) { g.rect(w - 28 + i * 4, h - 23, 4, 4, ['#c65a54', '#e8c34a', '#7ec069'][i]); }
  g.rect(w - 12, h - 18, 9, 11, ln(g, '#d8b58a')); g.rect(w - 11, h - 17, 7, 9, '#d8b58a'); g.rect(w - 10, h - 20, 5, 3, '#c9a06a');
...
export function drawInnFront(g, w, h, BB, glass, night, an = NOAN) {
  ...
  g.rect(12, -44, 10, 20, ln(g, '#9a8874')); g.rect(13, -43, 8, 18, '#9a8874'); g.rect(13, -43, 8, 3, '#b5a592');                                 // comignolo
  ...
  const staffaI = ln(g, '#8f887a');
  g.rect(w - 10, 24, 12, 2, staffaI); g.rect(w - 2, 26, 1, 4, staffaI);
  g.rect(w - 10 + sw, 30, 14, 12, ln(g, '#d9b98a')); g.rect(w - 9 + sw, 31, 12, 10, '#d9b98a'); g.rect(w - 6 + sw, 33, 5, 6, '#c9a06a'); g.rect(w - 6 + sw, 33, 5, 2, '#f3ecda');
}
export function drawBarberFront(g, w, h, BB, glass, night, an = NOAN) {
  ...
  const px0 = w / 2 + 12;
  g.rect(px0 - 1, h - 42, 10, 36, ln(g, '#f3ecda')); g.rect(px0, h - 41, 8, 34, '#f3ecda');
...
export function drawTailorFront(g, w, h, BB, glass, night, an = NOAN) {
  ...
  roof(g, 3, -6, w - 6, 22, BB);
  /* tenda a smerlo */
  g.rect(4, 19, w - 8, 8, ln(g, '#b06a8c')); g.rect(5, 20, w - 10, 6, '#b06a8c');
  ...
  g.rect(7, 33, 30, 24, ln(g, '#f3ecda')); g.rect(8, 34, 28, 22, '#f3ecda'); g.rect(10, 36, 24, 18, glass);
  ...
  for (let i = 0; i < 3; i++) { g.rect(w - 22 + i * 6, h - 30 + i * 2, 6, 22 - i * 2, ln(g, '#8fd0a0')); g.rect(w - 21 + i * 6, h - 29 + i * 2, 4, 20 - i * 2, ['#8fd0a0', '#e2604f', '#5a86c8'][i]); }
}
export function drawLabFront(g, w, h, BB, glass, night, an = NOAN) {
  ...
  const tx = w - 30;
  g.rect(tx - 1, -30, 24, 34, ln(g, '#a8a898')); wallFace(g, tx, -30, 22, 34, '#a8a898', 'stone');
  g.rect(tx - 4, -38, 30, 9, ln(g, '#5f7a52')); g.rect(tx - 3, -37, 28, 7, '#5f7a52'); g.rect(tx - 3, -37, 28, 2, '#78966a');
  g.rect(tx + 5, -24, 12, 12, ln(g, '#ffe08a')); g.rect(tx + 6, -23, 10, 10, night ? '#ffe08a' : '#8fd0e6'); g.rect(tx + 10, -23, 2, 10, '#5a5248'); g.rect(tx + 6, -19, 10, 2, '#5a5248');
  g.rect(tx + 8, -50, 4, 12, ln(g, '#8f9aa3')); g.rect(tx + 9, -49, 2, 10, '#8f9aa3');                                                              // antenna
  const blink = step(an, 450, 5) === 0;                                // la lucina in cima lampeggia
  g.rect(tx + 8, -54, 4, 4, ln(g, '#ff6a5a')); g.rect(tx + 9, -53, 2, 2, blink ? '#ff6a5a' : '#8a3a32');
  if (blink) g.rect(tx + 6, -56, 8, 8, 'rgba(255,110,90,.25)');
  g.rect(12, -18, 8, 20, ln(g, '#8f887a')); g.rect(13, -17, 6, 18, '#8f887a'); smoke(g, 16, -22, an, '170,235,180', an === NOAN);  // camino col fumo verde
  windowBox(g, 10, 34, 16, 14, glass, null, night);
  door(g, w / 2, h - 5, '#5f7a52', '#3f5434', true);
  /* casse di fiale fuori */
  g.rect(w - 22, h - 18, 16, 10, ln(g, '#a97a4c')); g.rect(w - 21, h - 17, 14, 8, '#a97a4c');
  for (let i = 0; i < 3; i++) { g.rect(w - 19 + i * 4, h - 22, 3, 6, ln(g, '#5fa04e')); g.rect(w - 19 + i * 4, h - 21, 2, 4, ['#5fa04e', '#5a86c8', '#c65a54'][i]); }
...
export function drawFurnitureFront(g, w, h, BB, glass, night, an = NOAN) {
  ...
  roof(g, 3, -6, w - 6, 22, BB);
  g.rect(6, 26, 30, 28, ln(g, '#6e4a2e')); g.rect(7, 27, 28, 26, '#6e4a2e'); g.rect(9, 29, 24, 22, glass);
  ...
  for (let i = 0; i < 3; i++) { g.rect(w - 14 + i * 3, h - 36 + i * 2, 4, 30 - i * 2, ln(g, '#b07c4a')); g.rect(w - 13 + i * 3, h - 35 + i * 2, 2, 28 - i * 2, ['#b07c4a', '#8a5f38', '#d0ae82'][i]); }
...
export function drawMuseumFront(g, w, h, BB, glass, night, an = NOAN) {
  const LNM = ln(g, '#e8e2d0');                                      // marmo scurito: il contorno del Museo
  g.rect(4, h - 2, w - 8, 6, 'rgba(20,14,8,.25)');
  wallFace(g, 6, 22, w - 12, h - 30, '#e8e2d0', 'marble');
  /* scalinata */
  for (let i = 0; i < 3; i++) { g.rect(2 + i * 3, h - 8 + i * 3, w - 4 - i * 6, 3, ln(g, '#d9d0bb')); g.rect(3 + i * 3, h - 8 + i * 3, w - 6 - i * 6, 2, ['#d9d0bb', '#e8e2d0', '#f4eedf'][i]); }
  /* frontone */
  ...
  for (let k = 0; k < 30; k++) {
    ...
    g.rect(x0m, -10 + k, ww, 1, (k === 29 || k < 2) ? LNM : '#e8e2d0');
    ...
    g.rect(x0m - 2, -10 + k, 2, 1, LNM); g.rect(x0m + ww, -10 + k, 2, 1, LNM);
    if (k > 0) { g.rect(x0m - 2, -11 + k, xPrev - x0m + 2, 1, LNM); g.rect(xPrev + wPrev, -11 + k, x0m + ww - xPrev - wPrev + 2, 1, LNM); }
  }
  ...
  g.rect(-4, 18, w + 8, 7, ln(g, '#c9a227')); g.rect(-3, 19, w + 6, 5, '#c9a227'); g.rect(-3, 19, w + 6, 1, '#f0d470');                             // architrave d'oro
  if (BB.snow) { g.rect(-4, 15, w + 8, 3, '#eef7fa'); }
  /* colonne scanalate */
  for (let i = 0; i < 6; i++) {
    const cx = 14 + Math.round(i * (w - 28) / 5);
    if (Math.abs(cx - w / 2) < 18) continue;
    g.rect(cx - 6, 25, 12, h - 33, ln(g, '#f4eedf')); g.rect(cx - 5, 25, 10, h - 33, '#f4eedf');
  ...
  /* portale */
  g.rect(w / 2 - 14, h - 40, 28, 32, ln(g, '#23232c')); g.rect(w / 2 - 12, h - 38, 24, 30, '#23232c'); g.rect(w / 2 - 12, h - 38, 24, 3, '#3a3a44');
...
export function drawHouseFront(g, w, h, BB, glass, night, an = NOAN) {
  ...
  g.rect(w - 26, -24, 10, 18, ln(g, '#9a8874')); g.rect(w - 25, -23, 8, 16, '#9a8874'); g.rect(w - 25, -23, 8, 3, '#b5a592');
  smoke(g, w - 21, -26, an, '225,220,210', an === NOAN);
  /* finestra tonda */
  const rx = w - 24, ry = 38;
  for (let yy = -8; yy <= 8; yy++) { const ww = Math.round(Math.sqrt(64 - yy * yy)); g.rect(rx - ww - 1, ry + yy, ww * 2 + 3, 1, ln(g, '#f3ecda')); }
